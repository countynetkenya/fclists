// FClist — Purchase Invoice dense list (AP lane).
// Chain an overdue/unpaid indicator onto the native Purchase Invoice list and add a jump to the dense
// AP board Script Report. EXTEND, never replace: we call fclists.extend_listview() (defined in
// fclists_lib.js) so ERPNext's own list config for Purchase Invoice — its indicators, buttons,
// add_fields — is preserved and merely augmented. A bare
// `frappe.listview_settings["Purchase Invoice"] = {...}` would clobber it (Frappe concatenates every
// app's list-js). Finding A.
(function () {
	"use strict";

	fclists.extend_listview("Purchase Invoice", {
		...
		onload: function (listview) {
			...
			listview.page.add_inner_button(__("Purchase Board"), function () {
				frappe.set_route("query-report", "FClist Purchase Invoice");
			});
		},
	});
})();
